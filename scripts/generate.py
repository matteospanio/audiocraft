...
def get_args() -> Args:
    ...
    parser.add_argument(
        "-t",
        "--times",
        help="Specify the number of times to generate each prompt. 1 by default.",
        default=1,
        type=int,
    )
    parser.add_argument(
        "-d",
        "--decoder",
        help="The decoder to use for the MusicGen model. Choose between 'encodec'"
        " and 'mdb'. Default is 'encodec'.",
        default="encodec",
        choices=["encodec", "mdb"],
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.decoder == "mbd":
        mdb = MultiBandDiffusion.get_mbd_musicgen()
    ...
    wavs, tokens = musicgen.generate(prompts, return_tokens=True)

    if args.decoder == "mbd":
        wavs_diff = mdb.tokens_to_wav(tokens)

    for idx, (wav, prompt) in enumerate(zip(wavs, prompts)):
        generated_audio = audio_write(
            f"{args.output}/{idx}_{prompt.replace(" ", "_").replace(",", "_")}",
            wav.cpu(),
